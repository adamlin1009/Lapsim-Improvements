...
%        .minPtsFrac      (default 0.25)
%        .minPtsAbs       (default 10)
%        .winFrac         (default 0.20)
%        .winMin          (default 6)
%        .winMax          (default 10)
%        .slopeTolAbs     (default 0.03)
%        .slopeTolRel     (default 0.20)
%        .resK            (default 3.0)
%        .resMinAbs       (default 0.15)
...
%
%   Toolbox-free; works with base MATLAB.
    ...
    % seed robust fit
    [b0, ~, r0, s0] = robust_line(x(1:minPts), y(1:minPts), opts.robustMethod, opts.robustC, opts.maxIter, opts.tol);
    slope_ref = b0(2);
...
%ROBUST_LINE IRLS robust linear regression for y ~ a + b x
    ...
    r = y - X*b; % final residuals
end

function s = mad_scale(r)
%MAD_SCALE Robust scale estimate from MAD
    s = 1.4826 * mad(r, 1);
    if ~isfinite(s) || s <= 0, s = std(r,0); end
    if ~isfinite(s) || s <= 0, s = 1.0; end
end
